...
void DifieHellmanEliptico(Punto& clave_publica_a, Punto& clave_secreta_a, Punto& clave_secreta_b, const std::vector<Punto>& puntos, int a, int p) {
  ...
  int clave_privada_a, clave_privada_b;
...
Punto CodificarMensaje(int mensaje, const std::vector<Punto> &puntos, int p) {
  int m_potencia_2{1};
  while (m_potencia_2 <= mensaje) {
    m_potencia_2 *= 2;
  }
  int num_h{p / m_potencia_2};
  for (int j{0}; j < num_h; j++) {
    for (const Punto &punto : puntos) {
      if (punto.getX() == (mensaje * num_h) + j) {
        return punto;
      }
    }
  }
...
Punto DescifradoElGamal(const Punto &mensaje_cifrado, const Punto &clave_privada, int a, int p) {
  Punto c1 = MultiplicacionPunto(clave_privada, 2, a, p);
  Punto c1_inverso = Punto(c1.getX(), (-c1.getY() + p) % p, p);
  return SumaPuntos(mensaje_cifrado, c1_inverso, a, p);
}