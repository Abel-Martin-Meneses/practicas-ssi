...
void ReadHexBlock(const std::string& label, unsigned char* block, bool& input_incomplete, size_t& length_input) {
  std::string input;
  bool is_valid = false;
  while (!is_valid) {
...
void EncryptCBC(unsigned char* state_1, unsigned char* state_2, const unsigned char* iv, const unsigned char* expanded_key, bool input_1_incomplete, bool input_2_incomplete, size_t length_input_1, size_t length_input_2) {
  if (input_1_incomplete) {
    int diff = 32 - length_input_1;
    int j{0};
    std::cout << diff;
    std::cout << std::endl;
    std::cout << "Bloque 1 sin rellenar: ";
    PrintBlock(state_1);
    std::cout << std::endl;
    for (size_t i = 16 - diff; i < 16; i++) {
      state_1[i] = iv[j++];
    }
    std::cout << "Bloque 1 de texto original (rellenado con IV): ";
    PrintBlock(state_1);
    std::cout << std::endl;
  }
  for (int i{0}; i < 16; i++) {
  ...
  PrintBlock(state_1);
  std::cout << std::endl;
  if (input_2_incomplete) {
    int diff = 16 - length_input_2;
    int j{0};
    for (size_t i = 16 - diff; i < 16; i++) {
      state_2[i] = state_1[j++];
    }
    std::cout << "Bloque 2 de texto original (rellenado con bloque 1 cifrado): ";
    PrintBlock(state_2);
    std::cout << std::endl;
  }
  for (int i{0}; i < 16; i++) {
