...
long long Carmichael(long long a, long long b) {
  return std::lcm(a, b);
}
...
bool ConfigurarClaves(long long& num_n, long long& num_e, long long& num_d) {
  ...
  long long lambda_n{Carmichael(p - 1, q - 1)};
  std::pair<bool, long long> primos_e{PrimosEuclidesExtendido(num_d, lambda_n)};
  ...
  std::cout << GREEN << "Se comprueba que " << num_d << " es primo con λ = " << lambda_n << "\n" << RESET; 
...
std::vector<long long> ProcesarTextoADecimal(std::string mensaje, long long num_n) {
  int tamaño_bloque{int(log(num_n) / log(26))}; 
  std::cout << GREEN << "\nComo n = " << num_n << ", se divide el texto en bloques de " << tamaño_bloque << " caracteres.\n" << RESET;
  
  mensaje.erase(std::remove_if(mensaje.begin(), mensaje.end(), ::isspace), mensaje.end());
  ...
  for (size_t i{0}; i < mensaje.size(); i += tamaño_bloque) {
    ...
    for (size_t k{0}; k < bloque_actual.size(); k++) {
      valor_bloque += (bloque_actual[k] - 'A') * pow(26, bloque_actual.size() - 1 - k);
    }
...
// Convierte Texto CIFRADO a bloques decimales (Tamaño j)
std::vector<long long> ProcesarTextoCifradoADecimal(std::string mensaje, long long num_n) {
  int tamaño_bloque_cifrado{int(log(num_n) / log(26)) + 1}; 
  
  mensaje.erase(std::remove_if(mensaje.begin(), mensaje.end(), ::isspace), mensaje.end());

  std::vector<long long> decimal_bloques;
  for (size_t i{0}; i < mensaje.size(); i += tamaño_bloque_cifrado) {
    std::string bloque_actual{mensaje.substr(i, tamaño_bloque_cifrado)};
    long long valor_bloque{0};
    for (size_t k{0}; k < bloque_actual.size(); k++) {
      valor_bloque += (bloque_actual[k] - 'A') * pow(26, bloque_actual.size() - 1 - k);
    }
...
// Convierte bloques descifrados de vuelta a Texto ORIGINAL (Tamaño j-1)
std::string ProcesarDecimalATexto(const std::vector<long long>& bloques, long long num_n) {
  int tamaño_bloque{int(log(num_n) / log(26))}; 
  std::string mensaje{""};

  for (long long valor : bloques) {
    std::string bloque_str{""};
    long long temp{valor};
    for (int k{0}; k < tamaño_bloque; k++) {
      char letra = char((temp % 26) + 'A');
      bloque_str = letra + bloque_str;
      temp /= 26;
    }
    mensaje += bloque_str;
  }
